	...
	prev_day(-1)
{
	...
	this->AddStyle( LV_OBJ_PART_MAIN, gui->getStyle() );


		/* Display time */
	this->timelabel = lv_label_create( this->getContainer(), NULL );
	...
	lv_obj_align( this->timelabel, this->getContainer(), LV_ALIGN_IN_TOP_MID, 0, 0 );
	lv_obj_set_auto_realign(this->timelabel, true);

	this->datelabel = lv_label_create( this->getContainer(), NULL );
	...
	lv_obj_align( this->datelabel, this->getContainer(), LV_ALIGN_IN_BOTTOM_MID, 0, 0 );
	lv_obj_set_auto_realign(this->datelabel, true);
